...
| `DEVLAB_RUNS_DRAIN_GRACE` | `5m` | Beim Stop (Selbst-Deploy-Neustart) sperrt devlabd neue Starts und lässt den laufenden Vorgang bis zu dieser Frist auslaufen, bevor es beendet (danach Carry-over, nie Kill). Ein währenddessen angeforderter Start wird eingereiht („Neustart läuft") und läuft nach dem Neustart automatisch. `TimeoutStopSec` in der Unit muss über dieser Frist liegen. |
