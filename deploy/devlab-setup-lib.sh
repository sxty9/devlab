...
#
# THE SERVE ROOT IS STATED HERE, as a plain `root * <path>` directive, and is NOT handed to `app_web` as a
# snippet argument. The shared snippet reads it back from the site's own root. The full reason — a measured
# outage on every host with a Caddy older than 2.7 — is recorded above setup_edge_caddyfile_text, where the
# snippet itself lives. Stating it here also puts the path in the generated file under a directive that
# says what the path IS, instead of a bare positional argument whose meaning only the snippet knows.
...
#                  deficiency nobody named. It takes NO ARGUMENT — see the section below, which is there
#                  to stop exactly that from being added back.
#   edge_absage    the answer for a name no application claimed.
#
# THE SERVE ROOT IS NOT A SNIPPET ARGUMENT, AND MUST NEVER BECOME ONE AGAIN. `app_web` is imported with no
# argument: the application's own site block states `root * <path>` (setup_app_route_text), the `file`
# matcher takes that site root as its default, and the 503 message names it back through
# `{http.vars.root}`. This is not a style preference — it is the fix for a MEASURED landscape-wide outage,
# and the reason is written here because the repair looks like something worth "modernising" away.
#
# WHAT HAPPENED. The snippet used to interpolate its path as `{args[0]}`. That bracket spelling for
# snippet arguments exists only from Caddy 2.7 on. An older Caddy does NOT refuse an unknown placeholder —
# it passes it through verbatim into the generated configuration, so the `file` matcher went looking for a
# directory literally named `{args[0]}`, never matched, and every hostname on that host fell into the 503
# branch with the placeholder printed in the message. Measured on a production host running Caddy 2.6.2
# (2026-08-10): every interface in the landscape was unreachable, on every host whose Caddy predates 2.7.
# There is a second, worse face of the same fault: an unresolvable root falls back to the caddy process's
# WORKING DIRECTORY, so a host whose working directory happens to hold an index.html served that unrelated
# page under a Holistic hostname instead of admitting the interface was missing. Both disappear here,
# because a literal absolute path can neither be misspelled by a version nor fall back to anywhere.
#
# WHY NOT SIMPLY `{args.0}`, the spelling 2.6.2 does understand. It was measured, and it works on both
# versions — and it was still REJECTED, because Caddy 2.11.4 logs "Placeholder {args.0} deprecated, use
# {args[0]} instead" on every single load, once per occurrence. That leaves the running software itself
# instructing the next maintainer to restore precisely the spelling that breaks the older host, and leaves
# this comment arguing with a warning the tool emits three times per reload. A deprecated spelling is also
# a spelling scheduled for removal, and its removal would silently reinstate this outage — silently,
# because the failure mode of an unknown placeholder is verbatim pass-through, never an error. Carrying no
# argument at all removes the disagreement instead of refereeing it: `root * <path>` is a plain directive
# every Caddy since 2.0 reads identically, and no version-conditional spelling is left for a future Caddy
# to drop. Measured on BOTH 2.6.2 and 2.11.4, and identical on each: face present → 200 with deep links,
# face genuinely absent → 503 naming the path, unclaimed hostname → 404.
#
...
(app_web) {
	...
	handle {
		respond "This application is installed on this host, but its interface is not: nothing is served from {http.vars.root}. The package's face did not arrive on this host, so there is nothing to show at the root of this name. No other application's interface is served in its place." 503
	}
}
