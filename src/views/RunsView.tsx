    ...
    <article className="mx-auto max-w-3xl px-8 py-7">
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex shrink-0 flex-wrap items-center justify-end gap-1.5">
          <RunTrigger id={run.id} kind="auto" onStarted={onRunStarted} />
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2 text-caption text-text-tertiary">
        ...
        {(run.model || run.effort || run.timeBudget) && (
          <span className="rounded bg-fill/10 px-1.5 py-0.5 font-medium text-text-secondary">
            {[run.model, run.effort, run.timeBudget && formatBudget(run.timeBudget)].filter(Boolean).join(' · ')}
          </span>
        )}
...
function RunEditor({
  ...
  const save = async () => {
    ...
    const body: RunInput = { name: name.trim(), enabled, model, effort, timeBudget: budget, schedule: cleanSchedule, axiomIds };
    try {
