...
import { ActiveRunsOverview, BlockedDeploysPanel, ExecutionList, ExecutionHistory, LiveExecution, RunsConfig, SlotsOverview, EmptyPlaceholder, RunTrigger, fmtDateTime, useActiveRun } from './MercuryExecutions';
      ...
      <div className="mt-4 flex flex-wrap items-center gap-2 text-caption text-text-tertiary">
        ...
        {(todo.model || todo.effort || todo.timeBudget) && (
          <span className="rounded bg-fill/10 px-1.5 py-0.5 font-medium text-text-secondary">
            {[todo.model, todo.effort, budgetLabel(todo.timeBudget)].filter(Boolean).join(' · ')}
          </span>
        )}
