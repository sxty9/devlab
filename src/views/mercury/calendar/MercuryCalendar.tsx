...
export function MercuryCalendar({
  ...
  const styleOf = (o: RunOccurrence): TypeStyle => (showTypes ? TYPE_STYLE[o.kind ?? 'auto'] : NEUTRAL_STYLE);
...
function OccurrenceRow({
  ...
  const body = (
    <>
      ...
      <div className="min-w-0 flex-1">
        <p className="truncate text-footnote text-text-primary">{occ.runTitle}</p>
                  ...
                  const chipBody = (
                    <>
                      <span className="flex items-center gap-1">
                        ...
                        {past && (
                          <span className={cn('ml-auto h-1.5 w-1.5 shrink-0 rounded-full', o.paused ? 'bg-warning' : o.succeeded ? 'bg-success' : 'bg-danger')} />
                        )}
                      </span>
                      <span className="truncate text-caption text-text-primary">{o.runTitle}</span>
                    </>
                  );
    ...
    <div className="rounded-card border border-separator bg-surface p-3">
      ...
      <div className="grid grid-cols-7 gap-1 text-center">
        ...
        {cells.map((c, i) => {
          ...
          let todo = 0;
          for (const o of occ) {
            if (showTypes && (o.kind ?? 'auto') === 'todo') todo += 1;
            else auto += 1;
          }
