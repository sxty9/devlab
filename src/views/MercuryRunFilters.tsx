...
/** The moment a run is next due: an auto run fires on its schedule (nextFireAt); a ToDo is due on its
 *  optional dueAt. Null when nothing is scheduled (a disabled run, or a ToDo run manually). */
function dueTime(run: Run): number | null {
  return parseTime(run.type === 'todo' ? run.dueAt : run.nextFireAt);
}
...
export function applyRunFilter(runs: Run[], f: RunFilter, now: number = Date.now()): Run[] {
  return runs.filter((run) => {
    if (f.idleSince !== 'any') {
      const last = parseTime(run.lastFiredAt);
