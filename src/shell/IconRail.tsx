...
/** The workspace tools, on the left. */
...
/** The AI, on the right — the symbol sits on the same side as the AI sidebar it opens
 *  (REQ-043.2). */
const RIGHT_ITEMS: RailItem[] = [{ id: 'claude', label: 'KI', Icon: ClaudeIcon }];

/** A 56px rail of tool toggles. Click an active tool to collapse its panel. The rail exists on
 *  both edges: the workspace tools on the left, the AI on the right beside its sidebar. */
export function IconRail({ side = 'left' }: { side?: 'left' | 'right' }) {
  const { activePanel, togglePanel, data, setOverlay } = useWorkspace();
  const changeCount = data.changes.length;
  const right = side === 'right';
  const items = right ? RIGHT_ITEMS : LEFT_ITEMS;

  return (
    <nav
      aria-label={right ? 'AI' : 'Tools'}
      className={cn(
        'dl-no-select flex w-14 shrink-0 flex-col items-center gap-1 bg-surface-sidebar py-2 [backdrop-filter:var(--material-blur-thin)]',
        right ? 'border-l border-separator' : 'border-r border-separator',
      )}
    >
      {items.map(({ id, label, Icon }) => {
        ...
        return (
          ...
          >
            <span
              className={cn(
                'absolute top-1/2 h-5 w-0.5 -translate-y-1/2 rounded-r bg-accent transition-opacity duration-fast',
