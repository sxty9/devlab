...
func (s *Server) StartScheduler(ctx context.Context) {
	...
	s.scheduler.SetPublisher(s.live) // the live-run set feeds the ONE change-stream (active topic)
...
func (x *runExecutor) Execute(ctx context.Context, run runs.Run, report func(resultID string)) (runs.ResultRef, error) {
	...
	saver := &liveSaver{do: func() {
		...
		x.s.live.Publish(live.TopicProgress) // the in-flight execution advanced → live followers refresh
	}}
	save := saver.force
