...
// executions, the live run) changes — from THIS session, another browser window, an automatic run, or
// a second instance sharing this process. The client (src/state/mercuryLive.tsx) opens exactly one of
// these per open Mercury surface and refetches the affected views on each notification, so the UI
// stays current without a resting poll.
//
// Why SSE and not polling or WebSocket: it is one long-lived GET, the browser's EventSource
// reconnects on its own after a drop (task requirement: an interrupted connection finds its way back),
// a closed surface closes the connection (no idle load), and there is no per-second request. It is
// half-duplex (server→client only), which is exactly the shape of "tell me when something changed".
//
