...
func (d *drainExec) Execute(ctx context.Context, run Run, report func(string)) (ResultRef, error) {
	report("res_" + run.ID)
	select {
...
func TestQueuedRunStartsAfterRestart(t *testing.T) {
	...
	if fe2.count() != 1 {
		t.Fatalf("the queued run did not auto-start after the restart (executed=%d)", fe2.count())
	}
	q, _, _ := store.Get("q")
	if q.StartPending {
...
func (e *resumingExec) Execute(_ context.Context, run Run, report func(string)) (ResultRef, error) {
...
func TestInterruptedExecutionResumesAfterRestart(t *testing.T) {
	...
	if worked := re.workedRepos(); len(worked) != 1 || worked[0] != "b" {
		t.Fatalf("resume did not continue at the same place: worked=%v (want only [b])", worked)
	}
	q, _, _ := store.Get("q")
	if !q.Done {
