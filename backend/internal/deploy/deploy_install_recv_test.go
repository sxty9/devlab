...
// THE SAME HAZARD ONE SHAPE LATER, and it must be closed in the same pass for the same reason.
//
// The shared `app_web` snippet no longer takes the serve root as a snippet ARGUMENT: an application's site
// block states `root * <path>` itself and imports the snippet bare (see edge_snippet_argument_test.go for
// why the argument had to go — an older caddy passed the placeholder through verbatim and every interface
// in the landscape answered 503). But the SHELL is written by this refresh while the app blocks are
// written at each application's DELIVERY. So a host that took the new shell while its blocks still passed
// an argument would have a snippet with no root at all, and would answer 503 for an interface that is
// demonstrably present — on EVERY caddy version, which is worse than the fault being repaired (both mixed
// combinations were measured at 503 with the face in place).
//
// The rewrite therefore happens HERE, in the same pass as the new shell, and needs nothing the file does
// not already carry: the path it used to pass as an argument becomes the path it states as its root.
func TestRefreshBringsAppBlocksOntoTheArgumentFreeSnippet(t *testing.T) {
	env, _, _, _, caddyMain, _ := provisionEnv(t)
	conf := env["DEVLAB_CADDY_CONF"]
	if err := os.MkdirAll(filepath.Dir(caddyMain), 0o755); err != nil {
		t.Fatal(err)
	...
	mustWrite(t, caddyMain, "# Managed by devlab-install-recv — the Holistic edge shell.\n"+testEdgeAddress+
		" {\n\timport "+conf+"/*.caddy\n}\n")
	// A root application's site block exactly as the PREVIOUS generator wrote it: the serve root handed to
	// the snippet as an argument, and no root stated in the block.
	oldBlock := "# Generated by devlab-install. The root application 'holistic' answers under the hostname this host declares\n" +
		"# for it; its whole /api/* space is its own, and its face is served at the root of that name.\n" +
		"http://dash.example.test:8080 {\n\timport holistic_service_routes\n\thandle /api/* {\n" +
		"\t\treverse_proxy 127.0.0.1:8781\n\t}\n\timport app_web /opt/holistic/www\n}\n"
	mustWrite(t, filepath.Join(conf, "apps", "holistic.caddy"), oldBlock)

	res := runInstallRecv(t, env) // the plain receiver refresh — the one-line hand step
	if res.exit != 0 {
		t.Fatalf("the refresh must succeed (exit 0), got %d\n%s", res.exit, res.out)
	}

	// DECISIVE: the rewritten block is byte for byte what the CURRENT generator writes for the same
	// application. That is the strongest assertion available here, and it is what makes this test
	// transitive: the generator's own output is measured against real caddy 2.6.2 and 2.11.4 elsewhere, so
	// a block that equals it inherits that proof instead of restating it.
	want := bashRenderTemplate(t, "setup_app_route_text holistic dash.example.test 8080 8781 /opt/holistic/www 1")
	got, err := os.ReadFile(filepath.Join(conf, "apps", "holistic.caddy"))
	if err != nil {
		t.Fatalf("the app block must still be on the apps shelf: %v\n%s", err, res.out)
	}
	if strings.TrimSpace(string(got)) != strings.TrimSpace(want) {
		t.Errorf("the migrated block must equal what the current generator writes.\n--- got ---\n%s\n--- want ---\n%s", string(got), want)
	}
	// …and the two halves of that shape, named individually, so a failure says WHICH half is missing.
	if !strings.Contains(string(got), "root * /opt/holistic/www") {
		t.Errorf("the block must state the serve root it used to pass as an argument:\n%s", string(got))
	}
	if strings.Contains(string(got), "import app_web /") {
		t.Errorf("the snippet must no longer be imported with an argument:\n%s", string(got))
	}
	if m, _ := filepath.Glob(filepath.Join(conf, "apps", "holistic.caddy.bak-*")); len(m) == 0 {
		t.Errorf("the block must be backed up before it is rewritten, so the run's rollback restores the host:\n%s", res.out)
	}
	if !strings.Contains(res.out, "state its own serve root") {
		t.Errorf("the rewrite must be reported, not performed silently:\n%s", res.out)
	}

	// IDEMPOTENT: a block already in the current shape is not touched again.
	res2 := runInstallRecv(t, env)
	if res2.exit != 0 {
		t.Fatalf("a second refresh must succeed, got %d\n%s", res2.exit, res2.out)
	}
	if strings.Contains(res2.out, "state its own serve root") {
		t.Errorf("a block already in the current shape has nothing to rewrite:\n%s", res2.out)
	}
	if again, _ := os.ReadFile(filepath.Join(conf, "apps", "holistic.caddy")); string(again) != string(got) {
		t.Errorf("a second refresh must leave the block exactly as it stands, got:\n%s", string(again))
	}
}
