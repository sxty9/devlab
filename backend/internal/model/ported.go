// Ported wire shapes of the IDE surface (field names as in the Bestand, semantics unchanged).
// The preview shape is gone (previews were removed); the former delivery-pipeline "Stage" struct
// is renamed RepoStage so the chain vocabulary owns the plain Stage name.
package model
...
// Worktree is a git worktree shown in the repo overview.
...
// RepoStage is one stage row of the repo overview (StructureView). Honest derivation or
// omission is the IDE building block's duty (B 1.4); the shape itself is ported.
...
// TermLine is a terminal scrollback line.
...
// AiAsk is a structured multiple-choice question the AI posed, rendered as clickable options in
// the chat bubble. It is aigentic's "interactive" protocol shape, surfaced verbatim.
type AiAsk struct {
	Questions []AiAskQuestion `json:"questions"`
}

// AiAskQuestion is one question: a short header chip, the question text, the offered options,
// and whether several may be chosen together.
...
// AssistantReply is what DevLab returns to the SPA after proxying an aigentic run. Model is the
// mandatory model label of every AI answer (labeling duty).
...
// PullRequestResult is what DevLab returns after opening (or adopting) a GitHub PR for a branch.
type PullRequestResult struct {
	...
	Existed bool   `json:"existed"` // true when an already-open PR was adopted rather than created
}
