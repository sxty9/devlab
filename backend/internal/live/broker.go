...
// (runs.Store, runs.Scheduler, runs.DeliveryStore, the axiom handlers) call Publish after a
// successful mutation. This keeps the broker importable from anywhere with no import cycle.
//
...
const (
	...
	// history restore) AND runtime state (schedule advanced, result attached, suspended, deferred,
	// done). Every such change flows through runs.Store, so publishing there covers them all at once.
	TopicRuns = "runs"
	// TopicActive — the set of live runs changed: a run started, its result id became known, it was
	// deferred, or it ended. Drives the "what is running right now" surface without a resting poll.
	TopicActive = "active"
	...
	// TopicDeliveries — the delivery ledger changed: a delivery was recorded, its PR outcome mirrored
	// (merged/closed), rolled back (counter-booked), a repo dev-reset, or a prod-delivery blocked/
	// resumed. Drives the Lieferungen surface and the blocked-deploys panel without a resting poll.
